                                 ...
                                 int nIterations, bool* pbStopFlag, const unsigned long nLoopKF, const bool bRobust)
{
    ...
    for(size_t i=0; i<vpKFs.size(); i++)
    {
        ...
        for(uint iter = 0;iter<pKF->mvKeysUn.size();++iter)
        {
            jslam_msgs::cv_keypoint cvk;
            cvk.x = pKF->mvKeysUn[iter].pt.x;
    ...
    if(p_msg_opt->keyframes.size() > 0){
        data_flag = true;
        std::cout << "data flag TRUE" << std::endl;
    }
